...
- Pas d'export AC.
- Pas de détection circuit/spéciale.
- Pas de tuilage.
- Pas de PBR (NeILF++).
- Pas de stitching.
- Pas d'API HTTP.
...
- [x] `uv run road2track ingest ./captures/balade_test` enregistre un projet et un segment via le workflow `ProcessProject`. Le workflow chaîne `ingest_session` (upload MinIO + persistance Postgres) puis `fuse_sensors` (fusion ARKit + GPS → trajectoire ENU géoréférencée, alignement Kabsch).
